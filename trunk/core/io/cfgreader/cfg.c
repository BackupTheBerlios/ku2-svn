...
kucode_t cfg_close( cfg_session_t *session )
{
	...
	fclose(session->cfgf);
	if ( session->qtree )
		ku_abtree_free(session->qtree, qtree_free);
	if ( session->vsp )
...
{
	va_list va;
	const char *cur = rules; // текущий символ
	int cont; // нужно ли обрабатывать дальше (continue?)
	pstart();

	if ( session->qtree == NULL ) {
		session->qtree = ku_abtree_create(cfg_cmpf, 0);
		if ( session->qtree == NULL )
			KU_ERRQ_PASS();
	}
	...
	for ( cont = 1; cont; cur++ ) {
		cfg_query_t *q;
		uint i;
		const char *b_id, // начало идентификатора
		...
		q->ptr = (void**)(((char*)q->fmt) + sizeof(char)
		                * (len_fmt + 1));
		...
		if ( ku_abtree_ins(session->qtree, q) != KE_NONE ) {
			dfree(q);
	...
	__cfg_inherited_error: va_end(va);
	KU_ERRQ_PASS();
}
...
static inline int cfg_parse_directive( cfg_session_t *session, const char *c,
                                       cfg_process_st *st )
{
	char *cur;
	pstart();
	...
	if ( !strcmp(cur, "section") ) // новая секция
	{
		...
		if ( *c != 0 )
			KU_ERRQNT_V(KE_SYNTAX, 0); // больше, чем один параметр!
		...
		if ( session->vsp == NULL ) {
			...
			if ( st->vspace_i == CFG_VSPACE_STACK_SIZE - 1 )
				KU_ERRQNT_V(KE_FULL, 0); // достигнута максимальная глубина секций
		}

		if ( vspace_adds(st->vspace[st->vspace_i], secname) != KE_NONE ) {
			preturn 0;
		}
		st->vspace[st->vspace_i + 1]
		                = vspace_gets(st->vspace[st->vspace_i], secname);
		st->vspace_i++;
		...
		if ( prfxlen + seclen + 2 >= CFG_BUFFER )
			KU_ERRQNT_V(KE_SYNTAX, 0); // переполнение префикса
		strcpy(st->prefix + prfxlen, secname);
		prfxlen += seclen;
		st->prefix[prfxlen] = '/';
		st->prefix[prfxlen + 1] = 0;
	} else if ( !strcmp(cur, "endsec") ) // конец секции
	{
		if ( *c != 0 )
			return 0; // не должно быть параметров!
		if ( st->vspace_i == 0 )
			KU_ERRQNT_V(KE_EMPTY, 0); // нечего закрывать
 ...
 */
static inline int cfg_parse_parameters( int has_rule, const char *c,
                                        cfg_process_st *st )
{
	uint i, len_fmt;
	char *cur;
		...
		if ( has_rule ) {
			...
			if ( (st->q->mode == 0) || (st->q->mode == 'd') ) {
				...
				if ( st->q->fmt[i] == VAL_STRING ) {
					if ( vlist_add(st->vlist, VAL_STRING,
					               ret.s) != KE_NONE ) {
						preturn 0;
					}
				} else {
					if ( vlist_add(st->vlist,
					               st->q->fmt[i], &ret)
					                != KE_NONE ) {
						preturn 0;
					}
...
kucode_t cfg_process( cfg_session_t *session )
{
	...
	while ( fgets(buf, CFG_BUFFER, session->cfgf) != NULL ) {
		...
		if ( st.q == NULL ) {
			if ( session->flags & CFG_STRICT )
				CFG_PROCESS_ERROR(KE_NOTFOUND)
			else if ( !(session->flags & CFG_DYNAMIC) )
				continue;
		}
		...
		if ( (st.vlist != NULL) && (st.vlist->datasz > 0) ) {
			...
			if ( vspace_addv_l(st.vspace[st.vspace_i], id_name,
			                   st.vlist) != KE_NONE )
				goto __cfg_inherited_error;
	...
	__cfg_error: if ( st.vlist != NULL )
		vlist_undef(st.vlist);
	KU_ERRQNT(__kucode);

	__cfg_inherited_error: if ( st.vlist != NULL )
		vlist_undef(st.vlist);
	KU_ERRQ_PASS();
}
